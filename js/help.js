  ...
  return [
    {
      ...
      intro: [
        ...
        '右のパネルは<b>タブ</b>で分かれています。'
        + '〔彫る〕ブラシとマスク / 〔トポロジ〕ポリゴンを増やす・作り直す / '
        + '〔モデル〕ポリゴンモデリング / 〔ツール〕デフォームやレイヤーなど / '
        + '〔表示〕マテリアルと視点 / 〔保存〕ブラウザ内保存。'
        + 'モードを切り替えると、そのモードでよく使うタブへ自動で移ります。',
      ],
      steps: [
      ...
      notes: [
        ...
        + '履歴は「前回から変わった頂点だけ」を覚えるので、ポリゴンが増えても回数は減りません。',
    ...
    },
    {
      ...
      steps: [
        ...
        'ビューポートを<b>クリック</b>で選択、<b>ドラッグ</b>で範囲選択。Shift で追加します。'
        + '<b>B</b> で矩形 ⇄ 投げ縄、<b>Alt+クリック</b>でエッジループになります。',
        '<b>E</b> 押し出し、<b>I</b> インセット、<b>Ctrl+B</b> ベベル、<b>Ctrl+R</b> ループカットで面を増やします。',
        '<b>G</b> 移動 / <b>R</b> 回転 / <b>S</b> 拡大縮小。'
        + '押した瞬間から<b>ボタンを押さずにマウスを動かすだけ</b>で変形します。'
        + 'クリックか Enter で確定、Esc か右クリックで取り消しです。',
        '<b>Tab</b> でスカルプトへ戻ると、三角形化して彫刻メッシュへ書き戻します。',
      ],
      items: [
        { name: '範囲選択（矩形 / 投げ縄）', sub: 'B で切り替え',
          desc: 'ドラッグで囲って選ぶ。矩形は四角、投げ縄はなぞった輪の中。'
            + '<b>既定では見えている面だけ</b>を選び、裏側は拾わない' },
        { name: 'X 線表示', sub: 'Alt+Z',
          desc: '入れると裏側まで突き抜けて選ぶ。反対側も一度に選びたいときに使う' },
      ...
      notes: [
        ...
        + 'モデリングへ入るとどちらも切れます。',
        '<b>G</b> / <b>R</b> / <b>S</b> は Blender と同じ「押した瞬間から動く」方式です。'
        + 'ボタンは押さなくてよく、マウスを動かすだけで移動 / 回転 / 拡大縮小します。'
        + '画面の上に出る帯に、いまの操作・固定した軸・動いた量が出ます。'
        + '<b>X</b> / <b>Y</b> / <b>Z</b> でその軸に固定（同じキーをもう一度で自由に戻る）、'
        + '<b>Ctrl</b> を押している間はきりの良い値に刻み、'
        + '変形中に <b>G</b> / <b>R</b> / <b>S</b> を押すと操作を乗り換えられます。'
        + '<b>クリック</b>か <b>Enter</b> で確定、<b>Esc</b> か<b>右クリック</b>で取り消すと'
        + '<b>1 ビットも違わない元の座標へ戻ります</b>。',
        'ハンドルを掴んで軸をきっちり指定したいときは <b>Shift+G</b>（またはパネルの'
        + '〔移動のギズモ〕など）でギズモを立てます。ZBrush のトランスポーズと同じ方式です。',
        '辺は<b>黒い線</b>で出ます。選択している辺は橙、形の縁（穴の境界）は赤です。'
        + '赤が出ていたら閉じていない場所があるという意味です。'
        + '<b>面モードでは選択した面を橙で塗ります</b>（線ではなく面の色で示します）。',
        '範囲選択は<b>見えている面だけ</b>を拾います（裏側は選ばれません）。'
        + '判定は面の向きで見ているので、他のパーツに隠れているところ'
        + '（胴の裏の手など）は弾けません。反対側もまとめて選びたいときは'
        + '<b>Alt+Z</b> で X 線表示を入れてください。',
        'ループカットは<b>1 回に 1 リングまで</b>です。直交する 2 リングは同じ四角を共有するので、'
        + '同時に切ると分割点が浮いて穴が開きます（Blender の Ctrl+R も 1 リングずつです）。',
        'ベベルは<b>辺の選び方が自由</b>です。ループになっていなくても、1 本だけでも通ります。'
        + '帯が途切れる端では、まわりの面 1 枚に頂点を 2 個差し込んで閉じます'
        + '（その面の辺が 1 本増えて、四角なら五角形になります。Blender と同じ処理です）。'
        + '断るのは穴の縁（境界の辺）と、まわりが一周していない頂点だけです。',
    ...
    {
      id: 'shading', title: '面の陰影（スムース / フラット / 自動）',
      intro: [
        '〔マテリアル〕→〔面の陰影〕で切り替えます。'
        + '<b>描画のときに使う法線を変えるだけ</b>なので、形もポリゴン数も変わりません'
        + '（切り替えても彫った内容は一切動きません）。',
      ],
      items: [
        {
          name: 'スムース', desc: '頂点の法線でなめらかに塗る（既定）。'
            + '隣の面と法線を共有するので、少ないポリゴンでも丸く見える',
        },
        {
          name: 'フラット', desc: '面ごとの法線で塗る。ポリゴンの面がはっきり見える。'
            + 'ローポリの形を確認したいときや、面の流れを見たいときに使う',
        },
        {
          name: '自動', desc: '角のきついところだけフラットにする。'
            + '〔自動スムースの角度〕より大きく折れているところが立ち、'
            + 'なだらかなところはなめらかなまま',
        },
      ],
      notes: [
        'フラットの面法線は<b>画面上の微分から求めています</b>。頂点を割らずに済むので、'
        + '切り替えの瞬間にポリゴンが増えたりピッキングがずれたりしません。',
        '自動の判定は「その面の法線が、そこの頂点法線から〔角度〕以上離れているか」です。'
        + '立方体は全部の稜線が立ち、球は全部なめらかなまま、彫った稜線はそこだけ立ちます。',
        '<b>Blender の自動スムースとは仕組みが違います。</b>'
        + 'Blender は頂点を面のグループごとに分けて法線を別々に持たせますが、こちらは'
        + '描画時に面ごとに判定します。そのため「片側だけなめらかにしたい」ような形'
        + '（円柱の側面と蓋の境目など）では、境目の近くに少しフラットな帯が出ます。'
        + '彫刻用のメッシュでは差が出にくいので、この方式を採っています'
        + '（頂点を割る方式は、彫るたびに面ごとの法線を作り直すことになり実時間では重すぎます）。',
      ],
    },
    {
      id: 'io', title: '保存と書き出し',
...
function shortcutModes(shortcuts) {
  ...
  groups.model.set('マウス', [
    ...
    { keys: '左ドラッグ', jp: '範囲選択（矩形 / 投げ縄。B で切り替え）' },
