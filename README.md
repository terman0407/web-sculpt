...
距離場カーネル（ダイナメッシュで最も重い部分）を AssemblyScript で WASM 化して比較した
（`assembly/dynafield.ts` / `npm run bench:wasm`）。結果は **1.27〜1.54 倍**で、
出力は JS 版とビット単位で完全一致（差分 0）。

ただし本体には組み込んでいない。理由は実測値に基づく:

- 対話中のフレームは既に vsync 張り付きで、CPU は 6.9 ms 中 0.64 ms しか使っていない。
  ここを 1.5 倍速くしても体感は変わらない。
- 効くのはダイナメッシュだけで、全体では約 1.3 倍（321 ms → 約 250 ms）。
  一方でビルドツールチェーン（AssemblyScript）と二重実装の保守コストが発生する。
- 同じ労力を「1 フレームの作業量に上限を設ける」に使ったほうが、
  過負荷時の p95 が 833 ms → 26 ms（32 倍）になり、体感への効果がはるかに大きかった。

なお GitHub Pages で WASM を動かすこと自体は問題ない:

- `.wasm` は静的ファイルとして `application/wasm` で配信される。
- **シングルスレッドなら制約なし。** 単一ファイル版でも base64 で埋め込めば
  `file://` から動く（fetch を使わないため）。
- **マルチスレッド（SharedArrayBuffer）は不可。** COOP/COEP レスポンスヘッダが必要だが
  GitHub Pages はヘッダを設定できない。Service Worker でヘッダを合成する回避策はあるが、
  初回リロードが必要になり `file://` では動かない。

再挑戦する場合は `npm install && npm run build:wasm` で `wasm/dynafield.wasm` を作り直せます
（2.2 KB）。ベンチは JS 版との出力一致もチェックするので、移植のリグレッションに気づけます。
